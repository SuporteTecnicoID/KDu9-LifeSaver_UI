#!/usr/bin/env bash
# ==============================================================================
# KDu9-LifeSaver_UI - v1.7 (ESTÁVEL: Visual RoyalBlue & Prevenção de Falhas)
# ==============================================================================
...
exibir_ajuda() {
    yad --title="KDu9-LifeSaver_UI v1.7 - Guia Técnico" \
...
while true; do
    DATA=$(yad --title="KDu9-LifeSaver_UI v1.7" \
        --window-icon="/usr/share/pixmaps/kdu9-lifesaver.png" \
...
yad --title="KDu9-LifeSaver_UI v1.7 - CONFIRMAÇÃO" --image="dialog-error" \
...
# Inicia o monitor em xterm tratando permissões administrativas e aplicando o visual customizado
if [ "$env" == "root" ] || [ "$USER" == "root" ]; then
    ...
    pkexec env DISPLAY=$DISPLAY XAUTHORITY=$XAUTHORITY xterm $X_OPTS -title "KDu9-LifeSaver_UI Monitor" -e "chmod 666 $MAPA; $CMD; echo ''; echo 'Cópia finalizada!'; read -n1" &
fi

yad --title="KDu9-LifeSaver_UI v1.7 - Sucesso" \
    --text="Cópia enviada para o terminal!\nDeseja abrir o visualizador gráfico de blocos agora?" \
    --image="dialog-information" \
    --button="Abrir ddrescueview:0" --button="Fechar:1"
...
if [ $? -eq 0 ]; then
    if command -v ddrescueview &> /dev/null; then
        ...
        yad --text="Erro: ddrescueview não foi encontrado no sistema." --image="dialog-warning"
    fi
fi
